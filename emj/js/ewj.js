...
(function() {
	$(function() {
		...
		$('.list_detail').hover(function() {
			...
			function createHs(sid){
				...
				for(var i = 0; i < data.history.length; i++) {
					...
					if(sid == data.history[i].sid) {
						var str = ''

						str +=
							'					<div class="rBodyPro">' +
							'						<a href="javascript:;">' +
							'							<img src="' + data.history[i].img + '" />' +
							'							<div class="s_name">' + data.history[i].title + '+</div>' +
							'						</a>' +
							'					</div>';
										...
										cookieMy.addCookie('hsSid',sid,7)
		...
		function cookiearrHs() { //把cookie值转换成数组(思路：通过arrsearch函数判断对应的cookie值是否已经存在，而且cookie是一个数组)
		if(cookieMy.getCookie('hsrc')) { //cartindex 存放cookie的索引名称
			hsSrc = cookieMy.getCookie('hsrc').split(',');
		} else {
			hsSrc = [];
		}
		if(cookieMy.getCookie('htitle')) { //cartnum   存放数量的cookie名称
			hstitle = cookieMy.getCookie('htitle').split(',');
		} else {
			hstitle = []
		}
		...
		//历史记录的操作
		$(document).on('click', '.crossGoods', function() {
			cookiearrHs()
			var mysid = parseInt($(this).find('.btn_box').attr('btnsid'))
			createHs(mysid)
		...
		if(cookieMy.getCookie('hsSid')){
			
	

			createHs(cookieMy.getCookie('hsSid'))
...
(function() {
	$(function() {
		...
		$('.headerNav_ul li').hover(function(){
					...
					$.ajax({
						...
						$.ajax({
							...
							$('.arrival_view').children('img').each(function(i) {
								$(this).attr('src', data.img_src[i])
								$(this).parents('.arrivalGoods').find('.btn_box').attr('Btnsid', data.Btnsid[i])
								$(this).attr('sid', data.sid[i])
					...
					$.ajax({
						...
						$.ajax({
							...
							$('.originDirectG').find('.crossGoods').each(function(i) {
								$(this).find('.cross_view').children('img').attr('src', data.img_src[i])
								$(this).find('.tag').children('img').attr('src', data.tag_logo[i])
								$(this).find('.btn_box').attr('btnsid', data.btnSid[i])
								$(this).find('.tag').children('img').attr('src', data.tag_logo[i])
								$(this).find('.jj_p1').children('img').attr('src', data.img_logo[i])
								$(this).find('.jj_box').children('span').html(data.title[i])
								$(this).find('.jj_p2').children('a').html(data.category[i])
